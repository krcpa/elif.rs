    ...
    fn validate_pattern(&self, text: &str) -> bool {
        if self.full_match {
            self.pattern.is_match(text)
                && self.pattern.find(text).is_some_and(|m| m.as_str() == text)
        } else {
            self.pattern.is_match(text)
        }
    }
...
impl ValidationRule for PatternValidator {
    ...
    fn parameters(&self) -> Option<Value> {
        ...
        params.insert(
            "pattern".to_string(),
            Value::String(self.pattern.as_str().to_string()),
        );
        params.insert("full_match".to_string(), Value::Bool(self.full_match));
        params.insert(
            "case_sensitive".to_string(),
            Value::Bool(self.case_sensitive),
        );
...
mod tests {
    ...
    async fn test_pattern_validator_basic() {
        ...
        assert!(validator
            ...
            .is_ok());
        assert!(validator
            .validate(&Value::String("World".to_string()), "name")
            .await
            .is_ok());

        // Invalid strings (contain numbers or special chars)
        assert!(validator
            .validate(&Value::String("hello123".to_string()), "name")
            .await
            .is_err());
        assert!(validator
    ...
    async fn test_pattern_validator_full_match() {
        let validator = PatternValidator::new(r"abc").unwrap().full_match(false); // Just find a match, don't require full match

        // Should match strings containing "abc"
        assert!(validator
            .validate(&Value::String("abcdef".to_string()), "text")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("123abc456".to_string()), "text")
            .await
            .is_ok());

        // Should not match strings without "abc"
        assert!(validator
            .validate(&Value::String("def".to_string()), "text")
            .await
            .is_err());
    }
    ...
    async fn test_pattern_validator_case_insensitive() {
        ...
        assert!(validator
            .validate(&Value::String("hello".to_string()), "greeting")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("HELLO".to_string()), "greeting")
        ...
        assert!(validator
            .validate(&Value::String("world".to_string()), "greeting")
            .await
            .is_err());
    }
    ...
    async fn test_pattern_validator_alphanumeric() {
        ...
        assert!(validator
            .validate(&Value::String("abc123".to_string()), "username")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("user123".to_string()), "username")
            .await
            .is_ok());

        // Should not allow special characters
        assert!(validator
            .validate(&Value::String("user@123".to_string()), "username")
            .await
            .is_err());
        assert!(validator
            .validate(&Value::String("user 123".to_string()), "username")
    ...
    async fn test_pattern_validator_phone_us() {
        ...
        for phone in valid_phones {
            let result = validator
                .validate(&Value::String(phone.to_string()), "phone")
                .await;
    ...
    async fn test_pattern_validator_hex_color() {
        ...
        // Valid hex colors
        assert!(validator
            .validate(&Value::String("#FF5733".to_string()), "color")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("#000000".to_string()), "color")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("#ffffff".to_string()), "color")
            .await
            .is_ok());
        ...
        assert!(validator
            ...
            .is_err()); // Missing #
        assert!(validator
            .validate(&Value::String("#FF57".to_string()), "color")
            .await
            .is_err()); // Too short
        assert!(validator
            .validate(&Value::String("#GG5733".to_string()), "color")
            .await
    ...
    async fn test_pattern_validator_uuid_v4() {
        ...
        // Invalid UUIDs
        assert!(validator
            .validate(&Value::String("550e8400-e29b-41d4-a716".to_string()), "id")
            .await
            .is_err()); // Too short
        assert!(validator
            .validate(&Value::String("not-a-uuid".to_string()), "id")
            .await
            .is_err()); // Invalid format
    }
    ...
    async fn test_pattern_validator_strong_password() {
        ...
        assert!(validator
            .validate(&Value::String("Password123!".to_string()), "password")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("MyP@ssw0rd".to_string()), "password")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("12345678".to_string()), "password")
            .await
            .is_ok());

        // Invalid passwords (too short)
        assert!(validator
            .validate(&Value::String("P@ss1".to_string()), "password")
            .await
            .is_err()); // Too short
        assert!(validator
            .validate(&Value::String("1234567".to_string()), "password")
            .await
            .is_err()); // Too short
    }
    ...
    async fn test_pattern_validator_ipv4() {
        ...
        assert!(validator
            .validate(&Value::String("192.168.1.1".to_string()), "ip")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("0.0.0.0".to_string()), "ip")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("255.255.255.255".to_string()), "ip")
            .await
            .is_ok());

        // Invalid IPv4 addresses
        assert!(validator
            .validate(&Value::String("256.1.1.1".to_string()), "ip")
            .await
            .is_err()); // Out of range
        assert!(validator
            .validate(&Value::String("192.168.1".to_string()), "ip")
            .await
            .is_err()); // Incomplete
        assert!(validator
            .validate(&Value::String("192.168.1.1.1".to_string()), "ip")
            .await
            .is_err()); // Too many octets
    }
    ...
    async fn test_pattern_validator_custom_message() {
        ...
        let result = validator
            .validate(&Value::String("hello".to_string()), "code")
            .await;
    ...
    async fn test_pattern_validator_invalid_type() {
        ...
        let result = validator
            .validate(&Value::Number(serde_json::Number::from(42)), "field")
            .await;
        assert!(result.is_err());
    ...
    async fn test_pattern_validator_zip_code_us() {
        ...
        // Valid ZIP codes
        assert!(validator
            .validate(&Value::String("12345".to_string()), "zip")
            .await
            .is_ok());
        assert!(validator
            .validate(&Value::String("12345-6789".to_string()), "zip")
            .await
            .is_ok());

        // Invalid ZIP codes
        assert!(validator
            .validate(&Value::String("1234".to_string()), "zip")
            .await
            .is_err()); // Too short
        assert!(validator
            .validate(&Value::String("123456".to_string()), "zip")
            .await
            .is_err()); // Too long without dash
        assert!(validator
            .validate(&Value::String("abcde".to_string()), "zip")
            .await
            .is_err()); // Non-numeric
    }
}
