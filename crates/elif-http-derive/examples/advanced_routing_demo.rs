...
use elif_http_derive::{routes, resource, group, get, post, put, delete, patch};
...
impl ComplexRoutes {
    ...
    pub fn get_comment(user_id: u32, post_id: u64, comment_id: u32) -> String {
        format!("Comment {} on post {} by user {}", comment_id, post_id, user_id)
    }
    ...
    pub fn upload_avatar(id: u32, file_data: Vec<u8>) -> String {
        format!("Uploaded avatar for user {} ({} bytes)", id, file_data.len())
    }
}
