...
impl TestController {
    pub fn handle_user_request(&self, user_id: u32) -> String {
        ...
        self.email_service.send_email("admin@example.com", &format!("User {} accessed", user_id));
        user
    }
}
...
mod tests {
    ...
    fn test_from_ioc_container_exists() {
        ...
        fn check_method_exists() {
            let _: fn(&IocContainer, Option<&elif_core::container::ScopeId>) -> Result<TestController, String> 
                = TestController::from_ioc_container;
        }
        check_method_exists();
    }
    ...
    fn test_scope_creation_error_propagation() {
        ...
        assert!(result.is_ok(), "Should be able to create controller without explicit scope");
        ...
        assert!(result2.is_ok(), "Should be able to create controller with explicit scope");
