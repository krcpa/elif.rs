...
mod tests {
    ...
    fn test_request_param_name_extraction() {
        ...
        let attrs: Vec<Attribute> = vec![parse_quote!(#[request])];

        assert_eq!(extract_request_param_name(&attrs), "req");

        // Test custom request parameter name
        let attrs_with_custom_name: Vec<Attribute> = vec![parse_quote!(#[request(custom_req)])];

        assert_eq!(
            extract_request_param_name(&attrs_with_custom_name),
            "custom_req"
        );
    }
    ...
    fn test_validation_logic_unit() {
        ...
        assert!(
            has_conflict_2,
            "String parameter with req name should conflict"
        );
    }
    ...
    fn test_body_param_validation() {
        use crate::params::{validate_body_param_consistency, BodyParamType, BodySpec};
        use syn::parse_quote;
        ...
        assert!(
            ...
            "Valid body parameter should pass validation"
        );
        ...
        let name_validation_result =
            validate_body_param_consistency(&invalid_name_spec, &valid_fn.sig);
        assert!(
            name_validation_result.is_err(),
            "Invalid parameter name should fail validation"
        );
        assert!(name_validation_result
            .unwrap_err()
            .contains("not found in function signature"));
        ...
        assert!(
            form_validation_result.is_ok(),
            "Valid form parameter should pass validation"
        );
        ...
        assert!(
            bytes_validation_result.is_ok(),
            "Valid bytes parameter should pass validation"
        );
        ...
        let type_validation_result =
            validate_body_param_consistency(&invalid_type_spec, &valid_fn.sig);
        assert!(
            type_validation_result.is_err(),
            "Type mismatch should fail validation"
        );
        assert!(type_validation_result
            .unwrap_err()
            .contains("expected 'HashMap<String, String>'"));
    }
    ...
    fn test_combined_attributes_detection() {
        use crate::params::BodyParamType;
        use crate::utils::{
            extract_body_param_from_attrs, has_body_attribute, has_request_attribute,
        };
        use syn::{parse_quote, Attribute};
        ...
        assert!(
            has_request_attribute(&combined_attrs),
            "Should detect #[request] attribute"
        );
        assert!(
            has_body_attribute(&combined_attrs),
            "Should detect #[body] attribute"
        );
