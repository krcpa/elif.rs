...
use elif_core::container::{
    IocContainer, ServiceToken, tokens::TokenRegistry, tokens::TokenBinding,
};
...
fn test_service_token_trait_metadata() {
    ...
    assert_eq!(EmailNotificationToken::token_type_name(), 
        "token_injection_tests::EmailNotificationToken");
    assert_eq!(EmailNotificationToken::service_type_name(), 
        "dyn token_injection_tests::EmailService");
    
    assert_eq!(DatabaseToken::token_type_name(),
        "token_injection_tests::DatabaseToken");
    assert_eq!(DatabaseToken::service_type_name(),
...
fn test_token_registry_basic_operations() {
    ...
    // Register tokens
    registry.register::<EmailNotificationToken, SmtpEmailService>().unwrap();
    registry.register::<DatabaseToken, PostgresService>().unwrap();
    ...
    assert_eq!(email_binding.token_type_name, "token_injection_tests::EmailNotificationToken");
    assert_eq!(email_binding.impl_type_name, "token_injection_tests::SmtpEmailService");
    
    let db_binding = registry.get_default::<DatabaseToken>().unwrap();
    assert!(db_binding.matches_token::<DatabaseToken>());
    assert_eq!(db_binding.impl_type_name, "token_injection_tests::PostgresService");
}
...
fn test_named_token_registration() {
    ...
    registry.register::<EmailNotificationToken, SmtpEmailService>().unwrap();
    registry.register_named::<EmailNotificationToken, SendGridEmailService>("sendgrid").unwrap();
    registry.register_named::<EmailNotificationToken, SmtpEmailService>("smtp_backup").unwrap();
    
    // Check default binding
    assert!(registry.contains::<EmailNotificationToken>());
    let default_binding = registry.get_default::<EmailNotificationToken>().unwrap();
    assert_eq!(default_binding.impl_type_name, "token_injection_tests::SmtpEmailService");
    ...
    let sendgrid_binding = registry.get_named::<EmailNotificationToken>("sendgrid").unwrap();
    assert_eq!(sendgrid_binding.impl_type_name, "token_injection_tests::SendGridEmailService");
    assert_eq!(sendgrid_binding.name, Some("sendgrid".to_string()));
    
    let smtp_backup_binding = registry.get_named::<EmailNotificationToken>("smtp_backup").unwrap();
    assert_eq!(smtp_backup_binding.impl_type_name, "token_injection_tests::SmtpEmailService");
...
fn test_multiple_tokens_same_service() {
    ...
    registry.register::<EmailNotificationToken, SmtpEmailService>().unwrap();
    registry.register::<MarketingEmailToken, SendGridEmailService>().unwrap();
    ...
    assert_eq!(notification_binding.impl_type_name, "token_injection_tests::SmtpEmailService");
    assert_eq!(marketing_binding.impl_type_name, "token_injection_tests::SendGridEmailService");
}
...
fn test_ioc_container_token_binding() {
    ...
    // Bind tokens
    container.bind_token::<EmailNotificationToken, SmtpEmailService>().unwrap();
    container.bind_token::<DatabaseToken, PostgresService>().unwrap();
...
fn test_ioc_container_named_token_binding() {
    ...
    // Bind named tokens
    container.bind_token_named::<EmailNotificationToken, SmtpEmailService>("smtp").unwrap();
    container.bind_token_named::<EmailNotificationToken, SendGridEmailService>("sendgrid").unwrap();
...
fn test_named_token_resolution() {
    ...
    let smtp_service = container.resolve_by_token_named::<EmailNotificationToken>("smtp").unwrap();
    assert_eq!(smtp_service.get_provider(), "smtp");
    
    let sendgrid_service = container.resolve_by_token_named::<EmailNotificationToken>("sendgrid").unwrap();
...
fn test_complete_token_workflow() {
    ...
    container.bind_token::<EmailNotificationToken, SmtpEmailService>().unwrap();
    container.bind_token::<DatabaseToken, PostgresService>().unwrap();
    container.bind_token_named::<MarketingEmailToken, SendGridEmailService>("marketing").unwrap();
    
    // 2. Build the container
    container.build().unwrap();
    
    // 3. Resolve services by token
    let notification_service = container.resolve_by_token::<EmailNotificationToken>().unwrap();
    let database = container.resolve_by_token::<DatabaseToken>().unwrap();
    let marketing_service = container.resolve_by_token_named::<MarketingEmailToken>("marketing").unwrap();
    
    // 4. Use the services
    notification_service.send("user@example.com", "Welcome", "Hello!").unwrap();
    let results = database.query("SELECT * FROM users").unwrap();
    assert!(!results.is_empty());
    marketing_service.send("leads@example.com", "Promo", "Special offer!").unwrap();
...
fn test_enhanced_token_validation() {
    ...
    let result = TokenBinding::validate_implementation::<EmailNotificationToken, SmtpEmailService>();
    assert!(result.is_ok());
...
fn test_registry_validation() {
    ...
    assert!(conflicts.is_empty(), "Binding conflicts found: {:?}", conflicts);
}
...
fn test_token_info_debugging() {
    ...
    assert_eq!(info.token_name, "token_injection_tests::EmailNotificationToken");
    ...
    assert!(info.implementation_types.contains(&"token_injection_tests::SmtpEmailService".to_string()));
    assert!(info.implementation_types.contains(&"token_injection_tests::SendGridEmailService".to_string()));
}