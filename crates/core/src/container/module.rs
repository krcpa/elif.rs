...
pub trait ServiceModule: Send + Sync where Self: 'static {
    /// Get the unique identifier for this module
    fn id(&self) -> ModuleId where Self: Sized {
        ModuleId::of::<Self>()
    }
    ...
    fn initialize(&self, container: &IocContainer) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), CoreError>> + Send + '_>> {
        let _ = container; // Default implementation does nothing
        Box::pin(async move { Ok(()) })
    }
    
    /// Cleanup module resources
    fn shutdown(&self, container: &IocContainer) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), CoreError>> + Send + '_>> {
        let _ = container; // Default implementation does nothing
        Box::pin(async move { Ok(()) })
    }
    ...
    /// Get module metadata
    fn metadata(&self) -> ModuleMetadata where Self: Sized {
        ModuleMetadata {
            id: self.id(),
...
impl ModuleRegistry {
    ...
    pub fn register_module<T: ServiceModule + 'static>(
        ...
        self.dependency_graph.insert(module_id.clone(), metadata.dependencies.clone());
    ...
    fn visit_module_for_ordering(
        ...
        if temp_visited.contains(module_id) {
            return Err(CoreError::InvalidServiceDescriptor {
                message: format!("Circular dependency detected involving module {}", module_id.name()),
            });
        }
            ...
            for dep_id in dependencies {
                ...
                if !self.modules.contains_key(dep_id) {
                    return Err(CoreError::ServiceNotFound {
                        service_type: format!("Module dependency {} for module {}", dep_id.name(), module_id.name()),
                    });
                }
    ...
    pub async fn initialize_all(&mut self, container: &IocContainer) -> Result<(), CoreError> {
        ...
        for module_id in order {
            ...
            let config = self.loaded_modules.get(&module_id)
                .map(|m| m.config.clone())
                .unwrap_or_default();
            
            // Initialize with timeout if specified
            let result = if let Some(timeout) = config.init_timeout {
                tokio::time::timeout(timeout, module.initialize(container)).await
    ...
    pub async fn shutdown_all(&mut self, container: &IocContainer) -> Result<(), CoreError> {
        ...
        for module_id in order {
            ...
            if let Err(e) = module.shutdown(container).await {
                eprintln!("Warning: Module {} shutdown failed: {}", module_id.name(), e);
    ...
    T: ServiceBinder
{
    ...
    pub fn add_modules<M: ServiceModule + 'static>(mut self, modules: Vec<M>) -> Result<Self, CoreError> {
...
mod tests {
    ...
    fn test_missing_dependency_validation() {
        ...
        let mut registry = ModuleRegistry::new();
        registry.register_module(ModuleWithMissingDep, None).unwrap();
    ...
    async fn test_modular_container_builder() {
        ...
        let result = builder
            .add_module(CoreModule).unwrap()
            .add_module(AuthModule).unwrap()
            .add_module(ApiModule).unwrap()
            .build();
