...
use crate::container::conventions::{ServiceConventions, ServiceMetadata, ServiceRegistry as ConventionServiceRegistry};
use crate::container::module::{ServiceModule, ModuleRegistry};
...
pub fn demo_phase6_features() -> Result<(), CoreError> {
    ...
    println!("✓ UserService lifetime: {:?}", conventions.get_lifetime_for_type("UserService"));
    println!("✓ UserRepository lifetime: {:?}", conventions.get_lifetime_for_type("UserRepository"));
    println!("✓ AppLogger lifetime: {:?}", conventions.get_lifetime_for_type("AppLogger"));
    
    // Register services using conventions
    let mut service_registry = ConventionServiceRegistry::new();
    
    let user_service_metadata = ServiceMetadata::new("UserService".to_string())
        .with_lifetime(ServiceScope::Singleton);
    service_registry.register_service(user_service_metadata);
    
    let user_repo_metadata = ServiceMetadata::new("UserRepository".to_string())
        .with_lifetime(ServiceScope::Scoped);
    service_registry.register_service(user_repo_metadata);
    
    println!("✓ Registered {} services with conventions", service_registry.all_services().len());
    println!();
    ...
    println!("✓ Module load order calculated: {} modules", load_order.len());
    ...
    for (i, module_id) in load_order.iter().enumerate() {
        if let Some(loaded_module) = module_registry.get_loaded_module(module_id) {
            println!("  {}. {} ({})", 
                ...
                loaded_module.metadata.version.as_deref().unwrap_or("no version")
            );
        }
    }
    println!();
    ...
    println!("✓ Container Status: {}", if container_info.is_built { "Built" } else { "Not Built" });
    ...
    match visualizer.visualize(VisualizationFormat::Ascii, style.clone()) {
        Ok(ascii_viz) => {
            ...
            println!("{}", ascii_viz.lines().take(10).collect::<Vec<_>>().join("\n"));
...
mod tests {
    ...
    fn test_service_conventions() {
        ...
        assert_eq!(conventions.get_lifetime_for_type("UserService"), ServiceScope::Singleton);
        assert_eq!(conventions.get_lifetime_for_type("UserRepository"), ServiceScope::Scoped);
        assert_eq!(conventions.get_lifetime_for_type("PaymentFactory"), ServiceScope::Transient);
        assert_eq!(conventions.get_lifetime_for_type("AppLogger"), ServiceScope::Singleton);
    }
