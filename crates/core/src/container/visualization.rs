...
impl DependencyVisualizer {
    ...
    pub fn from_container(container: &IocContainer) -> Self {
        let descriptors = container.registered_services()
            .into_iter()
    ...
    fn generate_mermaid(&self, style: VisualizationStyle) -> Result<String, CoreError> {
        ...
        for descriptor in &self.descriptors {
            ...
            if style.color_by_lifetime {
                ...
                writeln!(mermaid, "    {}[\"{} {}\"]::{}", 
                    node_id, lifetime_indicator, service_name, class_name).unwrap();
            } else {
                writeln!(mermaid, "    {}[\"{} {}\"]", 
                    node_id, lifetime_indicator, service_name).unwrap();
            }
        }
    ...
    fn generate_ascii(&self, style: VisualizationStyle) -> Result<String, CoreError> {
        ...
        for root in roots {
            self.generate_ascii_tree(root, &style, &mut ascii, 0, "", &mut visited, style.max_depth)?;
        }
    ...
    fn generate_ascii_tree(
        ...
        if visited.contains(service_id) {
            writeln!(output, "{}├── {} (circular)", prefix, self.format_service_name(service_id, style)).unwrap();
            return Ok(());
        }
        ...
        let service_display = if let Some(desc) = descriptor {
            if style.show_lifetimes {
                format!("{} ({:?})", self.format_service_name(service_id, style), desc.lifetime)
            } else {
    ...
    fn generate_json(&self, style: VisualizationStyle) -> Result<String, CoreError> {
        ...
        for descriptor in &self.descriptors {
            ...
            if style.show_lifetimes {
                service_data.insert("lifetime".to_string(), serde_json::Value::String(
                    format!("{:?}", descriptor.lifetime)
                ));
            }
            
            let deps: Vec<serde_json::Value> = descriptor.dependencies
                .iter()
        ...
        if style.include_stats {
            ...
            stats.insert("total_services".to_string(), serde_json::Value::Number(
                serde_json::Number::from(self.descriptors.len())
            ));
            
            let total_deps: usize = self.dependency_graph.values()
                .map(|deps| deps.len())
                .sum();
            stats.insert("total_dependencies".to_string(), serde_json::Value::Number(
                serde_json::Number::from(total_deps)
            ));
            
            json_data.insert("statistics".to_string(), serde_json::Value::Object(
