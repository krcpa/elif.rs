...
mod tests {
    ...
    use crate::container::{DependencyGraph, IocContainerBuilder, ServiceBinder, ServiceScope};
    ...
    fn test_dependency_graph_validation() {
        ...
        let logger_pos = sorted
            .iter()
            .position(|id| id.type_id == std::any::TypeId::of::<FileLogger>())
            .unwrap();
        let db_pos = sorted
            .iter()
            .position(|id| id.type_id == std::any::TypeId::of::<PostgresDatabase>())
            .unwrap();
        let service_pos = sorted
            .iter()
            .position(|id| id.type_id == std::any::TypeId::of::<UserService>())
            .unwrap();
    ...
    fn test_circular_dependency_detection() {
        ...
        let service_b_descriptor =
            ServiceDescriptor::bind_named::<PostgresDatabase, PostgresDatabase>("B")
        ...
        let descriptors = vec![
            service_a_descriptor,
            service_b_descriptor,
            service_c_descriptor,
        ];
        ...
        if let Err(CoreError::CircularDependency {
            path,
            cycle_service,
        }) = result
        {
    ...
    fn test_factory_with_configuration() {
        ...
        builder.bind_factory::<PostgresDatabase, _, _>(|| Ok(PostgresDatabase::default()));
