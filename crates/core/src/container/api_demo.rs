...
mod tests {
    ...
    fn test_named_services_api() {
        ...
        // Resolve by name
        let primary = container.resolve_named::<PostgresUserRepository>("primary").unwrap();
        let backup = container.resolve_named::<PostgresUserRepository>("backup").unwrap();
    ...
    fn test_performance_characteristics() {
        ...
        for i in 0..100 {
            let _logger = container.resolve_named::<FileLogger>(&format!("logger_{}", i)).unwrap();
        }
        let duration = start.elapsed();
        
        // Should resolve 100 services quickly (< 10ms)
        assert!(duration.as_millis() < 10, "Resolution took too long: {:?}", duration);
