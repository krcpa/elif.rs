...
impl CoreError {
    ...
    pub fn with_hint(mut self, hint: impl Into<String>) -> Self {
        if let Self::Api { hint: ref mut h, .. } = self {
            *h = Some(hint.into());
        }
        self
    }
...
impl From<CoreError> for ApiError {
    fn from(error: CoreError) -> Self {
        match error {
            CoreError::Api { code, message, hint } => Self { code, message, hint },
            ...
            CoreError::ServiceNotFound { service_type } => {
                Self::new("SERVICE_NOT_FOUND", format!("Service not found: {}", service_type))
            }
