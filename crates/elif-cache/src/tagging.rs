...
impl TagRegistry for MemoryTagRegistry {
    async fn tag_key(&self, key: &str, tags: &[&str]) -> CacheResult<()> {
        ...
        for tag in tags {
            ...
            self.key_to_tags.entry(key.clone()).or_default().insert(tag);
        }
    ...
    R: TagRegistry,
{
        ...
        Fut: std::future::Future<Output = T>,
    {
        ...
        if let Some(cached_bytes) = self.cache.get(key).await? {
            let value: T =
                serde_json::from_slice(&cached_bytes).map_err(CacheError::Serialization)?;
            return Ok(value);
        }
        ...
        self.cache
            .put_with_tags(key, bytes, Some(ttl), tags)
            .await?;
...
mod tests {
    ...
    async fn test_memory_tag_registry() {
        ...
        // Tag a key
        registry
            .tag_key("user:1", &["users", "active"])
            .await
            .unwrap();
        registry.tag_key("user:2", &["users"]).await.unwrap();
        registry
            .tag_key("post:1", &["posts", "active"])
            .await
            .unwrap();
