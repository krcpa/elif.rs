...
use crate::{CacheBackend, CacheError, CacheKey, CacheResult, CacheTag};
...
impl PatternStrategy {
    ...
    pub fn matches(&self, key: &str) -> CacheResult<bool> {
        match self {
            ...
            PatternStrategy::Prefix(prefix) => Ok(key.starts_with(prefix)),
            PatternStrategy::Suffix(suffix) => Ok(key.ends_with(suffix)),
            PatternStrategy::Contains(substring) => Ok(key.contains(substring)),
        }
    }
}
...
impl InvalidatableEntry {
    ...
    pub fn should_expire(&self) -> bool {
        self.time_policies
            .iter()
            .any(|policy| policy.should_expire(self.created_at))
    }
    ...
    async fn find_dependent_keys(&self, dependency_keys: &[&str]) -> CacheResult<Vec<CacheKey>> {
        ...
        for (key, entry) in entries.iter() {
            if entry
                .dependencies
                .iter()
                .any(|dep| dep_set.contains(dep.as_str()))
            {
...
mod tests {
    ...
    async fn test_memory_invalidation_manager() {
        ...
        backend
            .put("user:1", b"data1".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("user:2", b"data2".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("post:1", b"data3".to_vec(), None)
            .await
            .unwrap();
        ...
        assert_eq!(
            backend.get("post:1").await.unwrap(),
            Some(b"data3".to_vec())
        );
    ...
    async fn test_dependency_invalidation() {
        ...
        // Put some test data in cache
        backend
            .put("parent", b"parent_data".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("child1", b"child1_data".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("child2", b"child2_data".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("independent", b"independent_data".to_vec(), None)
            .await
            .unwrap();
        ...
        let invalidated = manager.invalidate_dependencies(&["parent"]).await.unwrap();
        ...
        assert_eq!(
            backend.get("parent").await.unwrap(),
            Some(b"parent_data".to_vec())
        );
        assert_eq!(
            backend.get("independent").await.unwrap(),
            Some(b"independent_data".to_vec())
        );
    }
    ...
    async fn test_time_based_invalidation() {
        ...
        // Put some test data in cache
        backend
            .put("temp1", b"temp1_data".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("temp2", b"temp2_data".to_vec(), None)
            .await
            .unwrap();
        backend
            .put("permanent", b"permanent_data".to_vec(), None)
            .await
            .unwrap();
        ...
        assert_eq!(
            backend.get("permanent").await.unwrap(),
            Some(b"permanent_data".to_vec())
        );
    }
    ...
    async fn test_parallel_invalidation_performance() {
        ...
        for i in 0..100 {
            backend
                .put(&format!("batch_key_{}", i), b"data".to_vec(), None)
                .await
                .unwrap();
        ...
        assert!(
            ...
            "Parallel invalidation took too long: {:?}",
            duration
        );
        ...
        for i in 0..100 {
            assert_eq!(
                backend.get(&format!("batch_key_{}", i)).await.unwrap(),
                None
            );
        }
    }
