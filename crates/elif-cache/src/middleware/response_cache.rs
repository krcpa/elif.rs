...
impl CachedHttpResponse {
    ...
    pub fn new(status: u16, headers: HashMap<String, String>, body: Vec<u8>, ttl: Duration) -> Self {
    ...
    pub fn to_elif_response(&self) -> CacheResult<ElifResponse> {
        ...
        response = response.header("X-Cache", "HIT")
            .map_err(|e| crate::CacheError::Backend(format!("Failed to add cache header: {}", e)))?;
...
impl<B: CacheBackend> HttpResponseCacheMiddleware<B> {
    ...
    pub fn should_cache_request(&self, request: &ElifRequest) -> bool {
        ...
        for exclude_pattern in &self.config.exclude_paths {
            if self.path_matches_glob(path, exclude_pattern) {
                debug!("Skipping cache for excluded path: {} (pattern: {})", path, exclude_pattern);
                return false;
            }
        }
        ...
        if let Ok(Some(cache_control)) = request.header_string("cache-control") {
            if cache_control.contains("no-cache") || cache_control.contains("no-store") {
                debug!("Skipping cache due to request Cache-Control: {}", cache_control);
                return false;
            }
        }
    ...
    pub fn handle_conditional_request(&self, request: &ElifRequest, cached: &CachedHttpResponse) -> Option<ElifResponse> {
            ...
            if let Some(ref etag) = cached.etag {
                if if_none_match == "*" || if_none_match.contains(etag) {
                    ...
                    let response = ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED)
                        .header("etag", etag).unwrap_or_else(|_| ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED))
                        .header("x-cache", "CONDITIONAL").unwrap_or_else(|_| ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED));
        ...
        if let Ok(Some(if_modified_since)) = request.header_string("if-modified-since") {
            if let Some(ref last_modified) = cached.last_modified {
                ...
                if if_modified_since == *last_modified {
                    debug!("Returning 304 Not Modified for Last-Modified match: {}", last_modified);
                    
                    let response = ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED)
                        .header("last-modified", last_modified).unwrap_or_else(|_| ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED))
                        .header("x-cache", "CONDITIONAL").unwrap_or_else(|_| ElifResponse::with_status(ElifStatusCode::NOT_MODIFIED));
    ...
    pub fn extract_response_data(&self, response: &Response) -> Option<(HashMap<String, String>, Vec<u8>)> {
            ...
            match self.cache.get::<CachedHttpResponse>(&cache_key).await {
                Ok(Some(cached_response)) => {
                    if cached_response.is_valid() {
                        ...
                        if let Some(conditional_response) = self.handle_conditional_request(&elif_request, &cached_response) {
...
mod tests {
    use super::*;
    use crate::{MemoryBackend, CacheConfig};
    ...
    fn test_cached_response_validity() {
        ...
        let valid_response = CachedHttpResponse::new(
            200, 
            headers.clone(), 
            body.clone(), 
        ...
        let mut expired_response = CachedHttpResponse::new(
            200, 
            headers, 
            body, 
            Duration::from_secs(300)
    ...
    fn test_builder_configuration() {
        ...
        assert!(middleware.config.vary_headers.contains(&"Authorization".to_string()));
        assert!(middleware.config.exclude_paths.contains(&"/api/private".to_string()));
