...
pub type WarmingProvider<T> = Box<
    dyn Fn() -> std::pin::Pin<
            Box<dyn std::future::Future<Output = CacheResult<Vec<(CacheKey, T)>>> + Send>,
        > + Send
        + Sync,
>;
...
impl<B: CacheBackend + 'static> CacheWarmingManager<B> {
    ...
    pub async fn register_task<T>(
        &self,
        task: WarmingTask,
        provider: WarmingProvider<T>,
    ) -> CacheResult<()>
    where
    ...
    pub async fn register_simple_task<T>(
        ...
        provider: Arc<
            dyn Fn() -> std::pin::Pin<
                    Box<dyn std::future::Future<Output = CacheResult<T>> + Send + 'static>,
                > + Send
                + Sync
                + 'static,
        >,
                    ...
                    for (task_id, task) in tasks_guard.iter() {
                        match &task.strategy {
                            WarmingStrategy::Scheduled(duration) => {
                                ...
                                let should_run =
                                    if let Some(last_warming) = stats_guard.last_warming {
                                        SystemTime::now()
                                            .duration_since(last_warming)
                ...
                for task_id in tasks_to_warm {
                    ...
                    tokio::spawn(async move {
                        ...
                        match result {
                            Ok(warming_result) => {
                                info!(
                                    "Warming task {} completed: {} entries warmed",
                                ...
                                // Update stats
                                {
                                    ...
                                    let new_avg_nanos = ((current_avg.as_nanos()
                                        * (total_runs - 1) as u128)
                                        + new_duration.as_nanos())
                                        / total_runs as u128;
                                    stats_guard.average_warming_duration = Duration::from_nanos(
                                        new_avg_nanos.min(u64::MAX as u128) as u64,
                                    );
                                }
                            }
                            Err(e) => {
    ...
    pub async fn warm_task(&self, task_id: &str) -> CacheResult<WarmingResult> {
        ...
        let result =
            Self::execute_warming_task(&self.backend, &self.tasks, &self.providers, task_id).await;
    ...
    pub async fn warm_startup(&self) -> CacheResult<Vec<WarmingResult>> {
        ...
        for (task_id, _) in sorted_tasks {
            match self.warm_task(&task_id).await {
                Ok(result) => {
                    info!(
                        "Startup warming completed for {}: {} entries warmed",
...
mod tests {
    ...
    async fn test_warming_result() {
        ...
        let failure = WarmingResult::failure(
            "task2".to_string(),
            5,
    ...
    async fn test_simple_warming_task_registration() {
        ...
        let provider = Arc::new(|| {
            Box::pin(async move { Ok("test_value".to_string()) })
                as std::pin::Pin<
                    Box<dyn std::future::Future<Output = CacheResult<String>> + Send + 'static>,
                >
        });

        let result = manager
            .register_simple_task(
    ...
    async fn test_warming_manager_startup() {
        ...
        let provider = Arc::new(|| {
            Box::pin(async move { Ok("startup_value".to_string()) })
                as std::pin::Pin<
                    Box<dyn std::future::Future<Output = CacheResult<String>> + Send + 'static>,
                >
        });
