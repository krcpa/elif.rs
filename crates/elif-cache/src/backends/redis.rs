...
impl RedisBackend {
    ...
    pub async fn new(config: RedisConfig) -> CacheResult<Self> {
        ...
        let connection_manager = ConnectionManager::new(client).await.map_err(|e| {
            CacheError::Backend(format!("Failed to create connection manager: {}", e))
        })?;
        ...
        R: Send + 'static,
    {
        ...
        let redis_result = tokio::time::timeout(self.config.command_timeout, operation(conn))
            .await
...
impl CacheBackend for RedisBackend {
        ...
        let result = self
            .with_connection(|mut conn| async move {
                match ttl {
                    ...
                    None => {
                        conn.set::<String, Vec<u8>, ()>(formatted_key, compressed_value)
                            .await
                    }
                }
            })
    ...
    async fn forget(&self, key: &str) -> CacheResult<bool> {
        ...
        let result = self
            .with_connection(|mut conn| async move { conn.del::<String, i32>(formatted_key).await })
            .await;
    ...
    async fn exists(&self, key: &str) -> CacheResult<bool> {
        ...
        let result = self
            .with_connection(
                |mut conn| async move { conn.exists::<String, bool>(formatted_key).await },
            )
            .await;
    ...
    async fn get_many(&self, keys: &[&str]) -> CacheResult<Vec<Option<Vec<u8>>>> {
        ...
        let result = self
            .with_connection(|mut conn| async move {
                conn.mget::<Vec<String>, Vec<Option<Vec<u8>>>>(formatted_keys)
                    .await
            })
            .await;
    ...
    async fn put_many(&self, entries: &[(&str, Vec<u8>, Option<Duration>)]) -> CacheResult<()> {
        ...
        match result {
            ...
            Err(e) => {
                error!(
                    "Redis pipeline PUT_MANY error for {} keys: {}",
                    entries.len(),
                    e
                );
                Err(e)
    ...
    async fn forget_many(&self, keys: &[&str]) -> CacheResult<usize> {
        ...
        match result {
            Ok(removed_count) => {
                if removed_count > 0 {
                    debug!(
                        "Successfully removed {} keys using batch DEL",
                        removed_count
                    );
                }
                Ok(removed_count)
    ...
    async fn stats(&self) -> CacheResult<CacheStats> {
        ...
        let redis_info = self
            .with_connection(|mut conn| async move { conn.info::<String>("memory").await })
            .await;
...
mod tests {
    ...
    async fn test_redis_backend_ttl() {
        ...
        backend
            .put("ttl_test", b"value".to_vec(), Some(Duration::from_secs(1)))
            .await
            .unwrap();
