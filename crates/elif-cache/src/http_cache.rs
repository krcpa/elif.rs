...
mod tests {
    ...
    async fn test_cached_response_validity() {
        let cached_response = CachedResponse::new(200, b"test").with_ttl(Duration::from_secs(300));

        assert!(cached_response.is_valid());

        // Create an expired response by manually setting old timestamp
        let mut expired_response =
            CachedResponse::new(200, b"test").with_ttl(Duration::from_secs(300));
        expired_response.cached_at = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs()
            - 400; // Cached 400 seconds ago
