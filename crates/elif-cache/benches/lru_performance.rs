...
use elif_cache::{MemoryBackend, CacheBackend, CacheConfig};
...
async fn benchmark_lru_operations(backend: &MemoryBackend, num_operations: usize) {
    ...
    for i in 0..num_operations {
        ...
        backend.put(&key, value, Some(Duration::from_secs(3600))).await.unwrap();
    }
                ...
                let handles = (0..10).map(|i| {
                    ...
                    tokio::spawn(async move {
                        for j in 0..100 {
                            ...
                            backend.put(&key, value, Some(Duration::from_secs(60))).await.unwrap();
                            backend.get(&key).await.unwrap();
                        }
                    })
                });
