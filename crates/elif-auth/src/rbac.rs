...
impl InMemoryRbacProvider {
    ...
    pub fn assign_role_to_user_mut(&mut self, user_id: &str, role_id: &str) -> AuthResult<()> {
        ...
        if !self.roles.contains_key(role_id) {
            return Err(AuthError::access_denied(format!("Role '{}' does not exist", role_id)));
        }

        let user_roles = self.user_roles.entry(user_id.to_string()).or_insert_with(Vec::new);
        
        // Check if user already has this role
        if user_roles.iter().any(|ur| ur.role_id == role_id && ur.is_valid()) {
...
impl RbacProvider for InMemoryRbacProvider {
    ...
    async fn assign_role_to_user(&self, _user_id: &str, _role_id: &str) -> AuthResult<()> {
        ...
        Err(AuthError::access_denied("Use assign_role_to_user_mut on the provider directly"))
    }
    ...
    async fn remove_role_from_user(&self, _user_id: &str, _role_id: &str) -> AuthResult<()> {
        ...
        // In a real implementation with mutable storage, this would work
        // For now, we need to use the mutable methods on the provider directly
        Err(AuthError::access_denied("Use remove_role_from_user_mut on the provider directly"))
    }
    ...
    U::Id: std::fmt::Display,
{
    ...
    async fn has_permission(&self, user: &Self::User, permission: &str) -> AuthResult<bool> {
        ...
        if parts.len() == 2 {
            ...
            let action = parts[1];
            let user_id = user.id().to_string();
            self.rbac_provider.user_has_permission(&user_id, resource, action, None).await
        } else {
            // If not in expected format, try direct permission lookup
            let user_id = user.id().to_string();
            self.rbac_provider.user_has_permission(&user_id, "*", permission, None).await
        }
    }
    ...
    async fn has_permission_with_context(
        ...
        let user_id = user.id().to_string();
        self.rbac_provider.user_has_permission(&user_id, resource, action, context).await
    }
...
mod tests {
    ...
    async fn test_rbac_authorization_adapter() {
        ...
        let has_permission_with_context = adapter.has_permission_with_context(
            &user, 
            "users", 
            "create", 
            None
    ...
    async fn test_role_assignment_and_checking() {
        ...
        assert!(!provider.user_has_permission(user_id, "users", "create", None).await.unwrap());
        ...
        assert!(provider.user_has_role(user_id, "admin").await.unwrap());
        assert!(provider.user_has_permission(user_id, "users", "create", None).await.unwrap());
        assert!(provider.user_has_permission(user_id, "users", "delete", None).await.unwrap());
        assert!(!provider.user_has_permission(user_id, "posts", "create", None).await.unwrap());
        ...
        // Test role removal
        provider.remove_role_from_user_mut(user_id, "admin").unwrap();
        assert!(!provider.user_has_role(user_id, "admin").await.unwrap());
        assert!(!provider.user_has_permission(user_id, "users", "create", None).await.unwrap());
    ...
    async fn test_permission_context_checking() {
        ...
        assert!(!provider.user_has_permission(user_id, "articles", "edit", None).await.unwrap());
        
        // Test with wrong context
        let mut wrong_context = HashMap::new();
        wrong_context.insert("owner".to_string(), serde_json::json!("other"));
        assert!(!provider.user_has_permission(user_id, "articles", "edit", Some(&wrong_context)).await.unwrap());
        
        // Test with correct context
        let mut correct_context = HashMap::new();
        correct_context.insert("owner".to_string(), serde_json::json!("self"));
        assert!(provider.user_has_permission(user_id, "articles", "edit", Some(&correct_context)).await.unwrap());
    }
