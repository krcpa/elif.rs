...
impl Model for BenchUser {
    ...
    fn to_fields(&self) -> ModelResult<HashMap<String, Value>> {
        ...
        fields.insert("created_at".to_string(), serde_json::to_value(&self.created_at)?);
        fields.insert("updated_at".to_string(), serde_json::to_value(&self.updated_at)?);
        Ok(fields)
    }
...
impl PrimaryKey<Uuid> for BenchUser {
    fn primary_key_type() -> PrimaryKeyType { PrimaryKeyType::Uuid }
}
            ...
            |b, &complexity| {
                b.iter(|| {
                    let mut builder = QueryBuilder::new()
                        ...
                        .join(black_box("profiles p"), black_box("p.user_id"), black_box("u.id"));
                    
                    // Add complexity through multiple conditions
                    for i in 0..complexity {
                        builder = builder
                            .where_eq(black_box(&format!("u.field_{}", i)), black_box(&format!("value_{}", i)))
                            .where_like(black_box(&format!("p.field_{}", i)), black_box(&format!("%{}%", i)));
                    }
...
fn bench_sql_generation(c: &mut Criterion) {
    ...
    let queries: Vec<(String, QueryBuilder)> = [1, 5, 10, 20, 50].iter().map(|&complexity| {
        let mut builder = QueryBuilder::new()
            .from("bench_users u")
        ...
        for i in 0..complexity {
            builder = builder
                ...
                .where_in(&format!("p.status_{}", i), &[format!("active_{}", i), format!("inactive_{}", i)]);
        }
    ...
    for &param_count in &[1, 10, 50, 100, 500] {
        group.bench_with_input(
            ...
            |b, &param_count| {
                ...
                let values: Vec<String> = (0..param_count)
                ...
                b.iter(|| {
                    ...
                    query_builder = query_builder.where_in(
                        black_box("status"), 
                        black_box(&values)
                    );
...
fn bench_query_cloning(c: &mut Criterion) {
    ...
    let queries: Vec<(String, QueryBuilder)> = [1, 5, 10, 25, 50].iter().map(|&size| {
        let mut builder = QueryBuilder::new()
            .from("bench_users u")
            .select("u.*");
        ...
        for i in 0..size {
            builder = builder
                .join(&format!("table_{} t{}", i, i), &format!("t{}.user_id", i), "u.id")
                .where_eq(&format!("u.field_{}", i), &format!("value_{}", i))
