...
use elif_orm::query::{QueryBuilder, acquire_query_builder, release_query_builder, QueryBuilderPool};
...
fn bench_sql_generation_comparison(c: &mut Criterion) {
    ...
    group.bench_function("simple_query_original", |b| {
        b.iter(|| {
            black_box(simple_query.to_sql())
        })
    });
    
    group.bench_function("simple_query_optimized", |b| {
        b.iter(|| {
            black_box(simple_query.to_sql_optimized())
        })
    });
    ...
    group.bench_function("complex_query_original", |b| {
        b.iter(|| {
            black_box(complex_query.to_sql())
        })
    });
    
    group.bench_function("complex_query_optimized", |b| {
        b.iter(|| {
            black_box(complex_query.to_sql_optimized())
        })
    });
...
fn bench_placeholder_generation(c: &mut Criterion) {
    ...
    for &count in &[1, 5, 10, 25, 50, 100] {
        ...
        group.bench_with_input(
            ...
            |b, &count| {
                b.iter(|| {
                    let placeholders = QueryBuilder::<()>::generate_placeholders_cached(black_box(count));
                    black_box(placeholders)
                })
            },
        );
