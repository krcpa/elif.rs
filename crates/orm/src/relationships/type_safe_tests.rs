...
impl Model for Profile {
    ...
    fn to_fields(&self) -> HashMap<String, serde_json::Value> {
        ...
        fields.insert(
            "bio".to_string(),
            serde_json::Value::String(self.bio.clone()),
        );
...
impl Model for Role {
    ...
    fn to_fields(&self) -> HashMap<String, serde_json::Value> {
        ...
        fields.insert(
            ...
            serde_json::Value::String(self.permissions.clone()),
        );
        fields
    }
...
impl Model for Tag {
    ...
    fn to_fields(&self) -> HashMap<String, serde_json::Value> {
        ...
        fields.insert(
            "slug".to_string(),
            serde_json::Value::String(self.slug.clone()),
        );
        fields
    }
...
mod container_tests {
    ...
    fn test_relationship_loading_states() {
        ...
        assert!(matches!(
            profile_rel.loading_state(),
            RelationshipLoadingState::Failed(_)
        ));
        assert!(profile_rel.try_get().is_err());
    ...
    fn test_relationship_mapping() {
        ...
        let mut profile_rel: TypeSafeRelationship<Option<Profile>> =
            TypeSafeRelationship::new(metadata);
        ...
        let bio_rel = profile_rel.map(|profile_opt| profile_opt.map(|p| p.bio));
...
mod inference_tests {
    ...
    fn test_individual_relationship_inference() {
        ...
        let posts_metadata = engine
            .infer_relationship::<Post>("posts", RelationshipType::HasMany)
            .unwrap();
    ...
    fn test_case_conversions() {
        ...
        assert_eq!(
            engine.to_camel_case("some_long_field_name"),
            "someLongFieldName"
        );
        ...
        assert_eq!(
            engine.to_pascal_case("some_long_field_name"),
            "SomeLongFieldName"
        );
    }
...
mod eager_loading_tests {
    ...
    fn test_relationship_data_variants() {
        ...
        match polymorphic_data {
            TypeSafeRelationshipData::Polymorphic {
                morph_type: Some(ref t),
                morph_id: Some(ref id),
                ..
            } => {
                assert_eq!(t, "Post");
                assert_eq!(id, "123");
