...
mod tests {
    ...
    impl ModelObserver<TestUser> for TestObserver {
        async fn creating(&self, model: &mut TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("creating: {}", model.name));
            Ok(())
        }

        async fn created(&self, model: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("created: {}", model.name));
            Ok(())
        }

        async fn updating(&self, model: &mut TestUser, original: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("updating: {} -> {}", original.name, model.name));
            Ok(())
        }

        async fn updated(&self, model: &TestUser, original: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("updated: {} -> {}", original.name, model.name));
            Ok(())
        }

        async fn saving(&self, model: &mut TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("saving: {}", model.name));
            Ok(())
        }

        async fn saved(&self, model: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("saved: {}", model.name));
            Ok(())
        }

        async fn deleting(&self, model: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("deleting: {}", model.name));
            Ok(())
        }

        async fn deleted(&self, model: &TestUser) -> Result<(), EventError> {
            self.events.lock().unwrap().push(format!("deleted: {}", model.name));
            Ok(())
        }
    }
    ...
    async fn test_observer_updating_event() {
        ...
        let mut updated = TestUser { name: "Updated User".to_string(), ..original.clone() };
    ...
    async fn test_observer_updated_event() {
        let observer = TestObserver::new();
        let original = TestUser::default();
        let updated = TestUser { name: "Updated User".to_string(), ..original.clone() };
