...
mod tests {
    ...
    async fn create_test_pool() -> Result<Arc<ManagedPool>, ModelError> {
        ...
        Err(ModelError::Connection("Test database not configured".to_string()))
    }
    ...
    async fn test_transaction_config_presets() {
        ...
        assert_eq!(serializable_config.isolation_level, Some(IsolationLevel::Serializable));
    ...
    async fn test_error_categorization() {
        ...
        for error_msg in database_errors {
            ...
            assert!(is_serialization_failure(&error), "Failed to detect serialization error: {}", error_msg);
        }
        ...
        for error_msg in non_serialization_errors {
            let error = ModelError::Database(error_msg.to_string());
            assert!(!is_serialization_failure(&error), "Incorrectly detected serialization error: {}", error_msg);
        }
    }
