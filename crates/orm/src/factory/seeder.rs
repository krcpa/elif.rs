...
impl<T: Model + Send + Sync, F: Factory<T> + Send + Sync> Seeder for FactorySeeder<T, F> {
    ...
    async fn run(&self, pool: &sqlx::Pool<sqlx::Postgres>) -> OrmResult<()> {
        tracing::info!("Running seeder: {} (creating {} records)", self.name, self.count);
        
        let models = self.factory.create_many(pool, self.count).await?;
        
        tracing::info!("Seeder {} completed: created {} records", self.name, models.len());
        Ok(())
    }
...
impl SeederManager {
    ...
    pub async fn run_for_environment(
        ...
        let mut applicable_seeders: Vec<&Box<dyn Seeder>> = self.seeders
            .iter()
        ...
        if !env.is_safe_for_seeding() {
            return Err(OrmError::Validation(
                format!("Environment '{}' is not safe for automatic seeding. Use explicit opt-in.", env.as_str())
            ));
        }
    ...
    pub async fn run_development(&self, pool: &sqlx::Pool<sqlx::Postgres>) -> OrmResult<()> {
        self.run_for_environment(pool, &Environment::Development).await
    }
    ...
    pub async fn run_production_force(&self, pool: &sqlx::Pool<sqlx::Postgres>) -> OrmResult<()> {
        ...
        tracing::warn!("Force running {} seeders in PRODUCTION environment", applicable_seeders.len());
...
pub struct CustomSeeder {
    ...
    priority: i32,
    dependencies: Vec<String>,
    run_fn: Box<dyn Fn(&sqlx::Pool<sqlx::Postgres>) -> std::pin::Pin<Box<dyn std::future::Future<Output = OrmResult<()>> + Send>> + Send + Sync>,
}
...
mod tests {
    ...
    async fn test_custom_seeder() {
        let seeder = CustomSeeder::new("test_seeder", |_pool| async {
            Ok(())
        });
