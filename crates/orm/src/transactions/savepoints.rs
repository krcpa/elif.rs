...
impl SavepointManager {
    ...
    pub async fn create_savepoint(&mut self, tx: &mut Box<dyn DatabaseTransaction>) -> ModelResult<String> {
    ...
    /// Release a savepoint
    pub async fn release_savepoint(&mut self, tx: &mut Box<dyn DatabaseTransaction>, savepoint_name: &str) -> ModelResult<()> {
    ...
    pub async fn rollback_to_savepoint(&mut self, tx: &mut Box<dyn DatabaseTransaction>, savepoint_name: &str) -> ModelResult<()> {
        let sql = format!("ROLLBACK TO SAVEPOINT {}", savepoint_name);
        tx.execute(&sql, &[])
            .await
            .map_err(|e| ModelError::Transaction(format!("Failed to rollback to savepoint: {}", e)))?;
