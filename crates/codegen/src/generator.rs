use crate::templates::{
    render_template, HANDLER_TEMPLATE, MIGRATION_TEMPLATE, MODEL_TEMPLATE, TEST_TEMPLATE,
};
use crate::writer::CodeWriter;
use elif_core::{specs::FieldSpec, ElifError, ResourceSpec};
...
impl<'a> ResourceGenerator<'a> {
    ...
    pub fn generate_handler(&self) -> Result<(), ElifError> {
        ...
        self.writer
            .write_preserving_markers(&handler_path, &content)?;
    ...
    pub fn generate_migration(&self) -> Result<(), ElifError> {
        let timestamp = std::time::SystemTime::now()
            ...
            .map_err(|e| ElifError::Codegen {
                message: format!("Time error: {}", e),
            })?
            .as_secs();

        let migration_path = self.project_root.join("migrations").join(format!(
            "{}_create_{}.sql",
            timestamp, self.spec.storage.table
        ));
    ...
    fn format_migration_field(&self, field: &FieldSpec) -> String {
        ...
        format!(
            "    {} {} {} {} {},",
            field.name, sql_type, pk, nullable, default
        )
        .trim()
        .to_string()
    }
