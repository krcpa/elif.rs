...
use elif_storage::{Storage, LocalBackend, LocalStorageConfig, UploadOptions};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📁 Using temporary storage directory: {}", storage_path.display());
    
    // Configure local storage
    let config = LocalStorageConfig::new()
        .with_root_path(storage_path);
    ...
    storage.put("documents/file1.txt", b"File 1 content", None).await?;
    storage.put("documents/file2.txt", b"File 2 content", None).await?;
    storage.put("documents/subdir/file3.txt", b"File 3 content", None).await?;
    ...
    // Copy a file
    let copy_result = storage.copy("documents/hello.txt", "backup/hello_backup.txt", None).await?;
    println!("  Copied file to: {} ({} bytes)", copy_result.path, copy_result.size);
    
    // Move a file
    let move_result = storage.move_file("documents/file1.txt", "archive/file1.txt", None).await?;
    println!("  Moved file to: {} ({} bytes)", move_result.path, move_result.size);
    ...
    println!("  Deleted {} files: {:?}", deleted_files.len(), deleted_files);
