...
impl StorageBackend for LocalBackend {
    async fn put(&self, path: &str, data: &[u8], options: Option<UploadOptions>) -> StorageResult<FileMetadata> {
        ...
        if let Some(opts) = &options {
            if !opts.overwrite && file_path.exists() {
                return Err(StorageError::Backend(format!("File already exists: {}", path)));
            }
        }
        ...
        if !metadata.metadata.is_empty() {
            ...
            let metadata_json = serde_json::to_string(&metadata.metadata)
                .map_err(|e| StorageError::Backend(format!("Failed to serialize metadata: {}", e)))?;
    ...
    async fn put_stream<S>(&self, path: &str, mut stream: S, options: Option<UploadOptions>) -> StorageResult<FileMetadata>
    where
            ...
            Box::pin(async move {
                ...
                while let Some(entry) = entries.next_entry().await? {
                    ...
                    if path.is_file() {
                        ...
                        // Get file metadata
                        if let Ok(Some(metadata)) = LocalBackend::metadata_for_path(&path, &relative_str).await {
                            files.push(metadata);
                        }
                    } else if path.is_dir() {
    ...
    async fn copy(&self, from: &str, to: &str, options: Option<UploadOptions>) -> StorageResult<FileMetadata> {
        ...
        if let Some(opts) = &options {
            if !opts.overwrite && to_path.exists() {
                return Err(StorageError::Backend(format!("File already exists: {}", to)));
            }
        }
    ...
    async fn move_file(&self, from: &str, to: &str, options: Option<UploadOptions>) -> StorageResult<FileMetadata> {
        ...
        self.metadata(to).await?
            .ok_or_else(|| StorageError::Backend("Failed to get metadata for moved file".to_string()))
    }
    ...
    async fn stats(&self) -> StorageResult<StorageStats> {
        ...
        let (available_space, used_space) = if let Ok(_metadata) = fs::metadata(&self.config.root_path).await {
...
impl LocalBackend {
    ...
    async fn metadata_for_path(file_path: &Path, relative_path: &str) -> StorageResult<Option<FileMetadata>> {
        if !file_path.exists() {
            return Ok(None);
        }
        ...
        // Generate ETag
        let modified_timestamp = fs_metadata.modified()
            .map_err(|e| StorageError::Io(e))?
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|e| StorageError::Backend(format!("Time error: {}", e)))?
            .as_secs();
...
mod tests {
    ...
    async fn test_copy_and_move() {
        ...
        // Test copy
        backend.copy("original.txt", "copy.txt", None).await.unwrap();
        ...
        // Test move
        backend.move_file("original.txt", "moved.txt", None).await.unwrap();
        assert!(!backend.exists("original.txt").await.unwrap());
    ...
    async fn test_list() {
        ...
        backend.put("dir1/file1.txt", b"content1", None).await.unwrap();
        backend.put("dir1/file2.txt", b"content2", None).await.unwrap();
        backend.put("dir2/file3.txt", b"content3", None).await.unwrap();
    ...
    async fn test_sanitize_path() {
        ...
        assert_eq!(backend.sanitize_path("../../../etc/passwd"), PathBuf::from("etc/passwd"));
        assert_eq!(backend.sanitize_path("./test/../file.txt"), PathBuf::from("test/file.txt"));
        assert_eq!(backend.sanitize_path("normal/path/file.txt"), PathBuf::from("normal/path/file.txt"));
        assert_eq!(backend.sanitize_path("/absolute/path"), PathBuf::from("absolute/path"));
    }
}