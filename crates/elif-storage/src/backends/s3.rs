...
impl S3Backend {
    ...
    pub async fn new(config: S3Config) -> StorageResult<Self> {
        let aws_config = if let (Some(key_id), Some(secret_key)) =
    ...
    fn get_public_url(&self, s3_key: &str) -> String {
        if let Some(cdn_domain) = &self.config.cdn_domain {
            ...
            format!(
                ...
                self.config.region, self.config.bucket, s3_key
            )
        } else {
            format!(
                "https://{}.s3.{}.amazonaws.com/{}",
    ...
    fn convert_s3_error(&self, error: aws_sdk_s3::Error) -> StorageError {
        match error {
            aws_sdk_s3::Error::NoSuchKey(_) => {
                StorageError::FileNotFound("File not found in S3".to_string())
            }
            aws_sdk_s3::Error::NoSuchBucket(_) => {
                StorageError::Configuration("S3 bucket not found".to_string())
            }
            aws_sdk_s3::Error::AccessDenied(_) => {
                StorageError::PermissionDenied("Access denied to S3 resource".to_string())
            }
...
impl StorageBackend for S3Backend {
        ...
        match result {
            Ok(response) => {
                let bytes = response
                    ...
                    .await
                    .map_err(|e| {
                        StorageError::Backend(format!("Failed to read S3 object body: {}", e))
                    })?
                    .into_bytes();
                Ok(Some(bytes))
            }
    ...
    async fn signed_url(&self, path: &str, expires_in: Duration) -> StorageResult<String> {
        ...
        let presigning_config = aws_sdk_s3::presigning::PresigningConfig::expires_in(expires_in)
            .map_err(|e| {
                StorageError::Backend(format!("Failed to create presigning config: {}", e))
            })?;
        ...
        let presigned_request = self
            ...
            .await
            .map_err(|e| {
                StorageError::Backend(format!("Failed to generate presigned URL: {}", e))
            })?;
    ...
    async fn delete_many(&self, paths: &[&str]) -> StorageResult<Vec<String>> {
        ...
        let delete_objects: Vec<_> = paths
            .iter()
            .map(|path| {
...
mod tests {
    ...
    fn test_s3_key_with_prefix() {
        let config = S3Config::new("test-bucket".to_string()).with_prefix("uploads/");
        let backend = S3Backend {
            client: Client::new(&aws_config::Config::builder().build()),
            config,
        };
    ...
    fn test_public_url_generation() {
        let config = S3Config::new("test-bucket".to_string()).with_region("us-west-2".to_string());
        let backend = S3Backend {
            client: Client::new(&aws_config::Config::builder().build()),
            config,
        };
    ...
    fn test_public_url_with_cdn() {
        let config =
            S3Config::new("test-bucket".to_string()).with_cdn("cdn.example.com".to_string());
        let backend = S3Backend {
            client: Client::new(&aws_config::Config::builder().build()),
            config,
        };
