    ...
    pub fn validate_mime_type(&self, mime_type: &str) -> StorageResult<()> {
        ...
        if let Some(allowed) = &self.config.allowed_mime_types {
            let is_allowed = allowed
                .iter()
                .any(|allowed_type| mime_type.starts_with(allowed_type));
            if !is_allowed {
                return Err(StorageError::UnsupportedFileType(format!(
    ...
    fn scan_for_dangerous_content(&self, filename: &str, content: &[u8]) -> StorageResult<()> {
        ...
        if content.len() >= 2 {
            match &content[0..2] {
                ...
                [0x7F, 0x45] if content.len() >= 4 && content[2..4] == [0x4C, 0x46] => {
                    // ELF
                    return Err(StorageError::Validation(
    ...
    match &content[0..4] {
        ...
        _ => {
            ...
            if content
                .iter()
                .take(1024)
                .all(|&b| b.is_ascii() && (b >= 32 || b == 9 || b == 10 || b == 13))
            {
...
mod tests {
    ...
    fn test_file_validator_extension() {
        let config =
            ValidationConfig::new().block_extensions(vec!["exe".to_string(), "bat".to_string()]);
        let validator = FileValidator::new(config);
    ...
    fn test_file_validator_filename() {
        ...
        assert!(validator
            .validate_filename("very_long_filename_that_exceeds_limit.txt")
            .is_err());
    ...
    fn test_detect_mime_from_content() {
        ...
        assert_eq!(
            detect_mime_from_content(&jpeg_data),
        ...
        assert_eq!(
            detect_mime_from_content(&png_data),
        ...
        assert_eq!(
            detect_mime_from_content(text_data),
            Some("text/plain".to_string())
        );
    ...
    fn test_dangerous_content_detection() {
        ...
        assert!(validator
            .scan_for_dangerous_content("test.txt", &pe_data)
            .is_err());

        // ELF executable
        let elf_data = [0x7F, 0x45, 0x4C, 0x46]; // ELF header
        assert!(validator
            .scan_for_dangerous_content("test.txt", &elf_data)
            .is_err());

        // Safe text
        let safe_text = b"This is just normal text content.";
        assert!(validator
            .scan_for_dangerous_content("test.txt", safe_text)
            .is_ok());

        // Dangerous script content
        let dangerous_script = b"powershell.exe -Command Invoke-Expression";
        assert!(validator
            .scan_for_dangerous_content("test.txt", dangerous_script)
            .is_err());
    }
}
