...
pub trait StorageBackend: Send + Sync {
    ...
    async fn get_stream(&self, path: &str) -> StorageResult<Option<Box<dyn futures::Stream<Item = Result<Bytes, std::io::Error>> + Send + Unpin>>>;
...
impl<B: StorageBackend> Storage<B> {
    ...
    pub async fn put(&self, path: &str, data: &[u8], options: Option<UploadOptions>) -> StorageResult<FileMetadata> {
        // Validate file
        if let Err(e) = validate_file_size(data.len() as u64, self.config.max_file_size) {
            return Err(e);
        }
        ...
        if let Err(e) = validate_file_type(&content_type, &self.config.allowed_types) {
            return Err(e);
        }
    ...
    pub async fn get_stream(&self, path: &str) -> StorageResult<Option<Box<dyn futures::Stream<Item = Result<Bytes, std::io::Error>> + Send + Unpin>>> {
...
fn detect_content_type(path: &str, data: &[u8]) -> String {
    ...
    if data.len() >= 4 {
        match &data[..4] {
            ...
            [0x52, 0x49, 0x46, 0x46] if data.len() >= 12 && &data[8..12] == b"WEBP" => return "image/webp".to_string(),
...
/// Validate file type
fn validate_file_type(content_type: &str, allowed_types: &Option<Vec<String>>) -> StorageResult<()> {
...
mod tests {
    ...
    fn test_detect_content_type() {
        ...
        // Test fallback
        assert_eq!(detect_content_type("unknown", &[0x00, 0x01, 0x02, 0x03]), "application/octet-stream");
    }
