...
pub enum ImageOperation {
    ...
    Resize { width: u32, height: u32, maintain_aspect_ratio: bool },
...
impl ImageProcessor {
    ...
    pub fn process_image(&self, data: &[u8], operations: &[ImageOperation]) -> StorageResult<Vec<u8>> {
        // Load the image
        ...
        for operation in operations {
            match operation {
                ImageOperation::Resize { width, height, maintain_aspect_ratio } => {
                ...
                ImageOperation::ConvertFormat { format } => {
                    if let Some(allowed) = &self.allowed_formats {
                        if !allowed.contains(format) {
                            return Err(StorageError::ImageProcessing(format!(
                                "Output format {:?} is not allowed", format
                            )));
                        }
                    }
                ...
                ImageOperation::Watermark { text, position, opacity: _ } => {
    ...
    pub fn get_image_info(&self, data: &[u8]) -> StorageResult<ImageInfo> {
        ...
        let dimensions = image::image_dimensions(&mut std::io::Cursor::new(data))
            .map_err(|e| StorageError::ImageProcessing(format!("Failed to read image dimensions: {}", e)))?;
    ...
    pub fn generate_thumbnail(&self, data: &[u8], max_width: u32, max_height: u32) -> StorageResult<Vec<u8>> {
        let operations = vec![
    ...
    pub fn optimize_for_web(&self, data: &[u8], target_format: ImageFormat) -> StorageResult<Vec<u8>> {
        let operations = match target_format {
            ...
            ImageFormat::WebP => vec![
                ImageOperation::ConvertFormat { format: ImageFormat::WebP },
            ],
            ImageFormat::Png => vec![
                ImageOperation::ConvertFormat { format: ImageFormat::Png },
            ],
            _ => {
                return Err(StorageError::ImageProcessing(
...
mod tests {
    ...
    fn test_image_operations() {
        let operations = vec![
            ...
            ImageOperation::Crop { x: 0, y: 0, width: 400, height: 300 },
            ImageOperation::ConvertFormat { format: ImageFormat::Jpeg },
