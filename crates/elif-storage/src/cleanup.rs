    ...
    pub async fn start(&self) -> StorageResult<tokio::task::JoinHandle<()>> {
        if self.running.swap(true, std::sync::atomic::Ordering::SeqCst) {
            return Err(StorageError::Configuration(
                "Cleanup manager already running".to_string(),
            ));
        }
        ...
        let handle = tokio::spawn(async move {
            ...
            info!(
                "Starting cleanup manager with {} directories to monitor",
                temp_directories.len()
            );
...
async fn cleanup_directory_with_stats(
    dir: &Path,
    max_age: Duration,
) -> StorageResult<CleanupStats> {
    if !dir.exists() {
...
fn cleanup_recursive<'a>(
    dir: &'a Path,
    cutoff_time: SystemTime,
    stats: &'a mut CleanupStats,
) -> std::pin::Pin<Box<dyn std::future::Future<Output = StorageResult<bool>> + Send + 'a>> {
    Box::pin(async move {
