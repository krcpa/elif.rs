...
pub async fn module(
    ...
    if !is_valid_module_name(name) {
        return Err(ElifError::validation("Module name must be PascalCase and end with 'Module' (e.g., UserModule)"));
    }
    ...
    let module_content = generate_module_content(
        name,
        providers.map(|p| p.split(',').collect()).unwrap_or_default(),
        controllers.map(|c| c.split(',').collect()).unwrap_or_default(),
...
pub async fn service(name: &str, to_module: &str, _trait_impl: Option<&str>) -> Result<(), ElifError> {
    println!("⚙️ Adding service '{}' to module '{}'", name, to_module);
    
    // Validate service name
    if !name.ends_with("Service") {
        return Err(ElifError::validation("Service name should end with 'Service' (e.g., EmailService)"));
    }
    
    // Check if target module exists
    let module_file = Path::new("src/modules").join(format!("{}.rs", to_snake_case(to_module)));
    if !module_file.exists() {
        return Err(ElifError::validation(&format!("Module '{}' not found. Create it first with: elifrs add module {}", to_module, to_module)));
    }
    ...
    println!("✅ Successfully added service '{}' to module '{}'", name, to_module);
...
pub async fn controller(name: &str, to_module: &str, crud: bool) -> Result<(), ElifError> {
    ...
    if !name.ends_with("Controller") {
        return Err(ElifError::validation("Controller name should end with 'Controller' (e.g., UserController)"));
    }
    ...
    println!("✅ Successfully added controller '{}' to module '{}'", name, to_module);
...
pub async fn middleware(name: &str, to_module: Option<&str>, debug: bool) -> Result<(), ElifError> {
    ...
    if !name.ends_with("Middleware") {
        return Err(ElifError::validation("Middleware name should end with 'Middleware' (e.g., AuthMiddleware)"));
    }
    ...
    println!("\n📖 Usage:");
    println!("   Add to controller: #[middleware(\"{}\")]", to_snake_case(name));
...
pub async fn seeder(name: &str) -> Result<(), ElifError> {
    ...
    let seeder_content = format!(r#"use elif_orm::{{Model, Database}};
use serde_json::json;
...
fn generate_module_content(
    ...
    let controllers_list = controllers.iter()
        .map(|c| format!("{}Controller", c.trim_end_matches("Controller")))
        .collect::<Vec<_>>()
        .join(", ");
        
    let providers_list = providers.iter()
        .chain(services.iter())
        .map(|p| format!("{}Service", p.trim_end_matches("Service")))
        .collect::<Vec<_>>()
        .join(", ");
    
    let content = format!(r#"use elif_http_derive::module;
...
async fn generate_controller_file(name: &str, _module_name: &str) -> Result<(), ElifError> {
    ...
    if !controller_dir.exists() {
        ...
        fs::write(controller_dir.join("mod.rs"), "// Controller declarations\n")?;
    }
    
    let controller_content = format!(r#"use elif_http::{{controller, get, post, put, delete, ElifRequest, ElifResponse, HttpResult}};
...
"#, to_snake_case(name.trim_end_matches("Controller")), name = name);
...
async fn generate_service_file(name: &str, _module_name: &str) -> Result<(), ElifError> {
    ...
    let service_content = format!(r#"use std::sync::Arc;
...
async fn generate_middleware_file(name: &str, debug: bool) -> Result<(), ElifError> {
    ...
    if !middleware_dir.exists() {
        ...
        fs::write(middleware_dir.join("mod.rs"), "// Middleware declarations\n")?;
    }
    ...
    let middleware_content = format!(r#"use elif_http::{{ElifRequest, ElifResponse, HttpResult}};
...
fn update_module_with_service(module_file: &Path, service_name: &str) -> Result<(), ElifError> {
    ...
    println!("   Add '{}' to the providers list in {}", service_name, module_file.display());
    
    Ok(())
}

fn update_module_with_controller(module_file: &Path, controller_name: &str) -> Result<(), ElifError> {
    // This is a simplified implementation - in a real implementation,
    // you'd parse the Rust AST and properly update the module macro
    let _content = fs::read_to_string(module_file)?;
    ...
    // For now, just print instructions
    println!("📝 Manual step required:");
    println!("   Add '{}' to the controllers list in {}", controller_name, module_file.display());
    
    Ok(())
}

fn update_module_with_middleware(_module_file: &Path, middleware_name: &str) -> Result<(), ElifError> {
    // This is a simplified implementation
    println!("📝 Manual step required:");
    println!("   Add middleware to your controller methods: #[middleware(\"{}\")]", to_snake_case(middleware_name));
