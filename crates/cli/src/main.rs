    ...
    match cli.command {
        ...
        Commands::Create { create_command } => {
            match create_command {
                CreateCommands::App { name, path, template, modules } => {
        ...
        Commands::Add { add_command } => {
            match add_command {
                AddCommands::Module { name, providers, controllers, services } => {
                    commands::add::module(&name, providers.as_deref(), controllers.as_deref(), services.as_deref()).await?;
                }
                AddCommands::Service { name, to, trait_impl } => {
        ...
        Commands::Dev { watch, profile, port, host, env } => {
            commands::dev::run(watch, profile, port, &host, &env).await?;
        }
        
        Commands::Check { comprehensive, module } => {
            commands::check::run(comprehensive, module.as_deref()).await?;
        }
        
        Commands::Doctor { fix_issues, verbose } => {
            commands::doctor::run(fix_issues, verbose).await?;
        }
        
        Commands::Test { unit, integration, watch, coverage, module } => {
        ...
        Commands::Build { release, target, optimizations } => {
            commands::build::run(release, &target, optimizations).await?;
        }
        
        Commands::Optimize { routes, assets, config } => {
        ...
        Commands::Version { version_command } => {
            match version_command {
                ...
                ApiVersionCommands::Deprecate { version, message, sunset_date } => {
                    commands::version::deprecate(&version, message.as_deref(), sunset_date.as_deref()).await?;
                }
