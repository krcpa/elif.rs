...
impl SanitizationMiddleware {
    ...
    pub fn permissive() -> Self {
        Self::new(SanitizationConfig {
            ...
            allowed_html_tags: vec![
                "b", "i", "em", "strong", "p", "br", "div", "span", "a", "img",
            ]
            .into_iter()
            .map(String::from)
            .collect(),
    ...
    pub fn sanitize_value(&self, value: &str) -> SecurityResult<String> {
        ...
        for pattern in &self.config.blocked_patterns {
            let regex = regex::Regex::new(pattern).map_err(|e| SecurityError::ConfigError {
        ...
        if self.config.enable_script_tag_removal {
            let script_regex = regex::Regex::new(r"<script[^>]*>.*?</script>").map_err(|e| {
...
mod tests {
    ...
    fn test_permissive_vs_strict() {
        ...
        assert!(strict
            .sanitize_value("<script>alert('xss')</script>")
            .is_err());
        assert!(permissive
            .sanitize_value("<script>alert('xss')</script>")
            .is_err());
