...
impl CorsMiddleware {
    ...
    fn handle_preflight(&self, request: &ElifRequest) -> SecurityResult<ElifResponse> {
        ...
        if let Err(e) = self.add_cors_headers(&mut response, 
...
mod tests {
    ...
    async fn test_cors_preflight_request() {
        ...
        headers.insert("origin".parse().unwrap(), "https://example.com".parse().unwrap());
        headers.insert("access-control-request-method".parse().unwrap(), "GET".parse().unwrap());
    ...
    async fn test_cors_origin_not_allowed() {
        ...
        let mut headers = ElifHeaderMap::new();
        headers.insert("origin".parse().unwrap(), "https://evil.com".parse().unwrap());
    ...
    async fn test_cors_method_not_allowed() {
        ...
        let mut headers = ElifHeaderMap::new();
        headers.insert("origin".parse().unwrap(), "https://example.com".parse().unwrap());
        headers.insert("access-control-request-method".parse().unwrap(), "DELETE".parse().unwrap());
