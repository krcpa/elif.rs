...
impl TestAssertions {
    ...
    pub fn assert_json_contains(actual: &JsonValue, expected: &JsonValue) -> TestResult<()> {
        if !json_contains(actual, expected) {
            return Err(TestError::Assertion {
                message: format!(
                    "JSON does not contain expected values:\nExpected to contain: {}\nActual: {}",
                    serde_json::to_string_pretty(expected).unwrap_or_default(),
                    serde_json::to_string_pretty(actual).unwrap_or_default()
                ),
            });
    ...
    pub fn assert_time_close(
        ...
        if diff > tolerance_seconds {
            return Err(TestError::Assertion {
                message: format!(
                    "Time difference too large: {} seconds (tolerance: {} seconds)",
...
mod tests {
    ...
    fn test_pattern_matching() -> TestResult<()> {
        ...
        let result =
            TestAssertions::assert_matches_pattern("invalid-email", r"^[^@]+@[^@]+\.[^@]+$");
        assert!(result.is_err());
    ...
    fn test_collection_assertions() -> TestResult<()> {
        ...
        TestAssertions::assert_any(
            &collection,
            |x| *x > 4,
            "Some items should be greater than 4",
        )?;
