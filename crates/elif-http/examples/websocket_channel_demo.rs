...
use elif_http::websocket::{
    ChannelPermissions, ChannelType, ConnectionId, ConnectionRegistry, WebSocketMessage,
};
use std::sync::Arc;
...
async fn main() {
    ...
    let general_id = channel_manager
        .create_channel("general".to_string(), ChannelType::Public, None)
        .await
        .unwrap();
    println!("   ✅ Created public channel 'general': {}", general_id);

    // Create a private channel with Alice as admin
    let private_id = channel_manager
        .create_channel(
    ...
    let protected_id = channel_manager
        .create_channel(
        ...
        .unwrap();
    println!(
        "   ✅ Created protected channel 'secret-project': {}",
        protected_id
    );
    println!();
    ...
    channel_manager
        .join_channel(general_id, alice_id, None, Some("Alice".to_string()))
        .await
        .unwrap();
    ...
    channel_manager
        .join_channel(general_id, bob_id, None, Some("Bob".to_string()))
        .await
        .unwrap();
    ...
    match channel_manager
        .join_channel(private_id, charlie_id, None, Some("Charlie".to_string()))
        .await
    {
    ...
    match channel_manager
        .join_channel(protected_id, bob_id, None, Some("Bob".to_string()))
        .await
    {
    ...
    let message_result = channel_manager
        .send_to_channel(
            general_id,
            alice_id,
        ...
        .unwrap();
    println!(
        "   ✅ Alice sent message to {} members in general channel",
        message_result.len()
    );
    ...
    let message_result = channel_manager
        ...
        .unwrap();
    println!(
        "   ✅ Bob sent message to {} members in general channel",
        message_result.len()
    );
    ...
    if let Some(general_channel) = channel_manager.get_channel(general_id).await {
        ...
        println!(
            "   📜 General channel has {} messages in history",
            history.len()
        );
    }
    println!();
    ...
    if let Some(general_channel) = channel_manager.get_channel(general_id).await {
        ...
        general_channel
            .update_member_permissions(bob_id, restricted_perms)
            .await
            .unwrap();
        ...
        general_channel
            .update_member_permissions(bob_id, ChannelPermissions::default())
            .await
            .unwrap();
    ...
    println!(
        "      • Private channels: {}",
        manager_stats.private_channels
    );
    println!(
        "      • Protected channels: {}",
        manager_stats.protected_channels
    );
    println!(
        "      • Total connections in channels: {}",
        manager_stats.total_connections_in_channels
    );
    ...
    channel_manager
        .leave_channel(general_id, bob_id)
        .await
        .unwrap();
    ...
    println!(
        "   📈 Updated stats: {} total channels, {} connections",
        updated_stats.total_channels, updated_stats.total_connections_in_channels
    );
