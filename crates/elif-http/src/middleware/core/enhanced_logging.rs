        ...
        Box::pin(async move {
            ...
            if config.correlation_ids && context.correlation_id != "disabled" {
                if let Err(e) = request
                    .headers
                    .add_header("x-elif-correlation-id", &context.correlation_id)
                {
            ...
            if config.structured {
                ...
                let log_msg = format!(
                    "← {:?} {} [{}] {}ms",
                    status, context.path, context.correlation_id, duration_ms
                );
            ...
            if config.correlation_ids && context.correlation_id != "disabled" {
                if let Err(e) =
                    response.add_header(&config.correlation_header, &context.correlation_id)
                {
...
mod tests {
    ...
    async fn test_enhanced_logging_middleware_v2() {
        ...
        headers.insert(
            ...
            "existing-correlation-123".parse().unwrap(),
        );
        ...
        let response = pipeline
            .execute(request, |_req| {
                Box::pin(async move { ElifResponse::ok().text("Created user") })
            })
            .await;
