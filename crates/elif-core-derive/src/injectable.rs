...
use quote::quote;
use syn::{
    parse::Result, parse_macro_input, Error, GenericArgument, Item, ItemStruct, PathArguments,
...
fn process_injectable_item(item: Item) -> Result<proc_macro2::TokenStream> {
    match item {
        Item::Struct(item_struct) => process_injectable_struct(&item_struct),
...
) -> Result<(Vec<DependencyInfo>, Vec<proc_macro2::TokenStream>)> {
            ...
            for field in &fields.named {
                if let Some(field_name) = &field.ident {
                    ...
                    if let Ok(dep_info) = analyze_field_type(&field.ty, field_name.clone()) {
                        ...
                        non_dependency_fields.push(generate_non_dependency_field_initializer(
                            field_name, &field.ty,
                        ));
                    }
                }
...
fn generate_non_dependency_field_initializer(
    field_name: &Ident,
    field_type: &Type,
...
fn analyze_field_type(field_type: &Type, field_name: Ident) -> Result<DependencyInfo> {
    match field_type {
        Type::Path(type_path) => analyze_type_path(type_path, field_name),
        _ => Err(Error::new_spanned(
            field_type,
...
fn analyze_type_path(type_path: &TypePath, field_name: Ident) -> Result<DependencyInfo> {
    if let Some(segment) = type_path.path.segments.last() {
        ...
        Err(Error::new_spanned(type_path, "Invalid type path"))
    }
}
...
fn extract_arc_inner_type(ty: &Type) -> Result<Type> {
    ...
    Err(Error::new_spanned(ty, "Expected Arc<T> type"))
}
