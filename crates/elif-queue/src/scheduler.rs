...
use crate::{Job, JobEntry, Priority, QueueError, QueueResult};
...
impl<'de> Deserialize<'de> for CronExpression {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        CronExpression::new(&data.expression).map_err(|e| serde::de::Error::custom(e.to_string()))
    }
}
...
impl<B: crate::QueueBackend + 'static> JobScheduler<B> {
    ...
    pub fn new(backend: std::sync::Arc<B>) -> Self {
        Self {
            backend,
            schedules: std::sync::Arc::new(parking_lot::RwLock::new(
    ...
    pub async fn get_dead_jobs(&self, limit: Option<usize>) -> QueueResult<Vec<crate::JobEntry>> {
        self.backend
            .get_jobs_by_state(crate::JobState::Dead, limit)
            .await
    }
    ...
    pub async fn clear_dead_jobs(&self) -> QueueResult<u64> {
        ...
        self.backend
            .clear_jobs_by_state(crate::JobState::Dead)
            .await
    }
}
...
pub struct JobCancellationManager {
    active_tokens:
        Arc<parking_lot::RwLock<std::collections::HashMap<crate::JobId, CancellationToken>>>,
}
...
impl JobMetrics {
    ...
    fn update_execution_time(&mut self, execution_time_ms: u64) {
        ...
        let completed_jobs =
            self.successful_jobs + self.failed_jobs + self.timeout_jobs + self.cancelled_jobs;
        if completed_jobs > 0 {
            ...
            self.avg_execution_time_ms +=
                (new_sample - self.avg_execution_time_ms) / completed_jobs as f64;
        }
    }
    ...
    fn update_retry_attempts(&mut self, attempts: u32) {
        if self.failed_jobs > 0 {
            ...
            // Use a numerically stable algorithm for running average: new_avg = old_avg + (new_sample - old_avg) / sample_count
            self.avg_retry_attempts +=
                (new_sample - self.avg_retry_attempts) / self.failed_jobs as f64;
        }
    }
}
...
pub struct JobMetricsCollector {
    ...
    active_executions:
        Arc<parking_lot::RwLock<std::collections::HashMap<crate::JobId, std::time::Instant>>>,
}
...
mod tests {
    ...
    fn test_cron_presets() {
        ...
        assert!(cron_presets::every_minute()
        ...
        assert!(cron_presets::weekdays_at_9am()
            .next_run_time(Utc::now())
            .is_some());
    }
    ...
    async fn test_atomic_clear_dead_jobs() {
        use crate::{JobEntry, MemoryBackend, Priority, QueueBackend, QueueConfig};
        ...
        let job1 = TestJob {
            message: "job1".to_string(),
        };
        let job2 = TestJob {
            message: "job2".to_string(),
        };
        let job3 = TestJob {
            message: "job3".to_string(),
        };
    ...
    async fn test_atomic_requeue_dead_job() {
        use crate::{JobEntry, JobState, MemoryBackend, Priority, QueueBackend, QueueConfig};
