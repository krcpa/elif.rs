...
mod tests {
    ...
    fn test_development_config() {
        let config = QueueConfigBuilder::development().build().expect("Failed to build config");
    ...
    fn test_production_config() {
        let config = QueueConfigBuilder::production().build().expect("Failed to build config");
        assert_eq!(*config.get_max_workers(), 8);
