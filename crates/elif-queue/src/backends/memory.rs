...
use crate::{JobEntry, JobId, JobState, QueueBackend, QueueError, QueueResult, QueueStats, JobResult, QueueConfig};
...
impl Ord for PriorityJobEntry {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match self.entry.priority().cmp(&other.entry.priority()) {
            Ordering::Equal => {
                match other.entry.run_at().cmp(&self.entry.run_at()) {
...
impl QueueBackend for MemoryBackend {
    async fn enqueue(&self, job: JobEntry) -> QueueResult<JobId> {
        ...
        if *self.config.get_max_queue_size() > 0 && self.jobs.len() >= *self.config.get_max_queue_size() {
            return Err(QueueError::Configuration(
                format!("Queue size limit exceeded: {}", *self.config.get_max_queue_size())
            ));
        }
    ...
    async fn get_jobs_by_state(&self, state: JobState, limit: Option<usize>) -> QueueResult<Vec<JobEntry>> {
        let mut jobs: Vec<JobEntry> = self.jobs
            .iter()
...
mod tests {
    ...
    async fn test_priority_ordering() {
        ...
        let low_job = TestJob { id: 1, message: "low".to_string() };
        let high_job = TestJob { id: 2, message: "high".to_string() };
        let normal_job = TestJob { id: 3, message: "normal".to_string() };
    ...
    async fn test_job_failure_and_retry() {
        ...
        let failed_jobs = backend.get_jobs_by_state(JobState::Failed, None).await.unwrap();
    ...
    async fn test_queue_size_limit() {
        ...
        // Enqueue up to limit
        for i in 1..=2 {
            let job = TestJob { id: i, message: format!("job {}", i) };
            let entry = JobEntry::new(job, None, None).unwrap();
            backend.enqueue(entry).await.unwrap();
        }
        
        // Third job should fail
        let job = TestJob { id: 3, message: "overflow job".to_string() };
        let entry = JobEntry::new(job, None, None).unwrap();
