...
use crate::{
    JobEntry, JobId, JobResult, JobState, QueueBackend, QueueError, QueueResult, QueueStats,
    RedisConfig,
};
...
impl RedisBackend {
    ...
    fn serialize_job(&self, job: &JobEntry) -> QueueResult<String> {
        serde_json::to_string(job).map_err(|e| QueueError::Serialization(e))
    }
    ...
    async fn process_delayed_jobs(&self) -> QueueResult<()> {
        ...
        let processed_count: i32 = conn
            ...
            .await
            .map_err(|e| {
                QueueError::Backend(format!("Failed to process delayed jobs atomically: {}", e))
            })?;
...
impl QueueBackend for RedisBackend {
    ...
    async fn complete(&self, job_id: JobId, result: JobResult<()>) -> QueueResult<()> {
        ...
        let (retry_score, is_delayed_retry) = if result.is_err() {
            ...
            let job_data: Option<String> = conn.get(&job_key).await.map_err(|e| {
                QueueError::Backend(format!("Failed to get job for retry calculation: {}", e))
            })?;
        ...
        let _: String = conn
            ...
            .await
            .map_err(|e| {
                QueueError::Backend(format!("Failed to complete job atomically: {}", e))
            })?;
    ...
    async fn remove_job(&self, job_id: JobId) -> QueueResult<bool> {
        ...
        let _: () = conn.zrem(&priority_key, &serialized).await.map_err(|e| {
            QueueError::Backend(format!("Failed to remove from priority queue: {}", e))
        })?;

        let _: () = conn.zrem(&delayed_key, &serialized).await.map_err(|e| {
            QueueError::Backend(format!("Failed to remove from delayed queue: {}", e))
        })?;
    ...
    async fn clear(&self) -> QueueResult<()> {
        ...
        for chunk in all_keys.chunks(BATCH_SIZE) {
            if !chunk.is_empty() {
                let _: () = conn.del(chunk).await.map_err(|e| {
                    QueueError::Backend(format!("Failed to clear queue batch: {}", e))
                })?;
            }
        }
    ...
    async fn stats(&self) -> QueueResult<QueueStats> {
        ...
        for state in [
            ...
            let state_key = self.get_state_key(&state);
            let count: u64 = conn.scard(&state_key).await.map_err(|e| {
                QueueError::Backend(format!("Failed to count jobs in state {:?}: {}", state, e))
            })?;
        ...
        stats.total_jobs = stats.pending_jobs
    ...
    async fn requeue_job(&self, job_id: JobId, mut job: JobEntry) -> QueueResult<bool> {
        ...
        match result {
            ...
            _ => Err(QueueError::Backend(
                "Unexpected result from requeue script".to_string(),
            )),
        }
    }
...
mod tests {
    ...
    async fn test_redis_atomicity_guarantees() {
        ...
        let job = TestJob {
            id: 100,
            message: "atomicity test".to_string(),
        };
    ...
    async fn test_redis_concurrent_delayed_job_processing() {
        ...
        for i in 1..=5 {
            let job = TestJob {
                id: i,
                message: format!("concurrent delayed job {}", i),
            };
            // Create job with delay in the past so it's immediately ready
            let entry =
                JobEntry::new(job, Some(Priority::Normal), Some(Duration::from_millis(1))).unwrap();
            backend.enqueue(entry).await.unwrap();
        }
