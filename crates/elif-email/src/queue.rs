...
use elif_queue::{
    Job, JobEntry, JobId, JobResult, JobState, Priority, Queue, QueueBackend, QueueError,
    QueueResult,
};
...
impl EmailJobProcessor {
    ...
    pub async fn process(&self, job: &EmailJob) -> JobResult<()> {
        ...
        info!(
            "Processing email job {} with priority {:?}",
            job.email.id, job.priority
        );
        ...
        let result = tokio::time::timeout(Duration::from_secs(job.timeout_seconds), async {
            // Wrap the provider call in a task that can handle panics
            let provider_clone = provider.clone();
            let email_clone = job.email.clone();
            match tokio::spawn(async move { provider_clone.send(&email_clone).await }).await {
                Ok(result) => result,
                Err(join_error) => {
                    if join_error.is_panic() {
                        Err(EmailError::provider(
                            "unknown",
                            "Provider panicked during send",
                        ))
                    } else {
                        Err(EmailError::provider(
                            "unknown",
                            "Provider task was cancelled",
                        ))
                    }
                }
            }
...
impl<B: QueueBackend> EmailQueueService<B> {
    ...
    pub async fn enqueue_scheduled(
                ...
                Ok(duration) => {
                    ...
                    if duration > max_delay {
                        return Err(QueueError::Configuration(format!(
                            "Scheduled time is too far in the future (max 30 days): {:?}",
                            duration
                        )));
                    }
                    duration
        ...
        let job = EmailJob::new(email);
        self.queue
            .enqueue_delayed(job, delay, Some(Priority::Normal))
            .await
    }
    ...
    pub async fn process_next_job(&self) -> QueueResult<bool> {
        if let Some(processor) = &self.processor {
            ...
            Err(QueueError::Configuration(
                "No email processor configured".to_string(),
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_email_priority_conversion() {
        ...
        assert_eq!(
            EmailPriority::from(Priority::Critical),
    ...
    async fn test_process_next_job_without_processor() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("No email processor configured"));
    }
