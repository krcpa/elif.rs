use crate::{config::AttachmentConfig, Attachment, EmailError};
...
pub fn validate_attachments(
    ...
    if total_size > config.max_total_size {
        return Err(EmailError::validation(
            ...
            format!(
                "Total attachments size too large: {} bytes (max: {} bytes)",
...
mod tests {
    ...
    fn test_compression_ratio_estimates() {
        ...
        assert_eq!(
            AttachmentCompressor::estimate_compression_ratio(&text_attachment),
            0.3
        );

        let image_attachment = Attachment::new("test.jpg", b"JPEG data".to_vec());
        assert_eq!(
            AttachmentCompressor::estimate_compression_ratio(&image_attachment),
            0.95
        );
    }
    ...
    fn test_attachment_validation() {
        ...
        let attachments = vec![Attachment::new("test.txt", b"Hello".to_vec())];
    ...
    fn test_compression_ratio_for_gzip_files() {
        ...
        assert_eq!(
            AttachmentCompressor::estimate_compression_ratio(&attachment),
            1.0
        );
