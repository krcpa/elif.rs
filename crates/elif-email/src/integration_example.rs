...
use crate::{
    Email, EmailJob, EmailJobProcessor, EmailPriority, EmailProvider, EmailQueueService,
...
impl crate::EmailProvider for MockEmailProvider {
    async fn send(&self, email: &Email) -> Result<crate::EmailResult, crate::EmailError> {
        info!(
            "MockEmailProvider '{}' processing email: {}",
            self.name, email.id
        );
        ...
        if self.should_fail {
            return Err(crate::EmailError::provider(
                "mock",
...
pub async fn run_integration_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    processor
        .register_provider("backup".to_string(), backup_provider)
        .await;

    // 3. Set up queue backend and service
    let backend = MemoryBackend::new(QueueConfig::default());
    let queue_service = Arc::new(EmailQueueService::new(backend).with_processor(processor.clone()));
    ...
    let job_ids = vec![
        queue_service
            .enqueue_with_priority(emails[0].clone(), EmailPriority::Normal)
            .await?,
        queue_service
            .enqueue_with_priority(emails[1].clone(), EmailPriority::Critical)
            .await?,
        queue_service
            .enqueue_with_priority(emails[2].clone(), EmailPriority::Low)
            .await?,
    ];
    ...
    let scheduled_job_id = queue_service
        .enqueue_scheduled(scheduled_email, send_at)
        .await?;
    info!("Scheduled email job: {}", scheduled_job_id);

    // 7. Queue a batch of emails
    let batch_emails: Vec<Email> = (0..5)
        .map(|i| {
            Email::new()
    ...
    for status in &delivery_statuses {
        info!(
            "Email {} delivery status: {:?}, attempts: {}",
            status.email_id,
            status.status,
            status.attempts.len()
        );
...
pub async fn run_failure_handling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let job_id = queue_service
        .enqueue_with_priority(email, EmailPriority::High)
        .await?;
